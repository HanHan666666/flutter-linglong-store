...
当前 GitHub Actions 采用四层结构：

- `ci.yml`
  - 仅用于 `pull_request` 轻量校验
- `nightly.yml`
  - 用于 nightly `amd64 + arm64` 主预发布
- `nightly-loong64.yml`
  - 用于在主 nightly 完成后异步补传 `loong64` 资产
- `release.yml`
  - 用于手动正式发版（`amd64 + arm64 + loong64(bundle/deb)`）

这四条工作流的职责必须保持分离，不要为了“少一个文件”把它们重新揉回一条 workflow。
...
- 每天固定生成一次最新 nightly `amd64 + arm64` 主预发布
...
- 在 `Nightly` workflow 成功完成后异步补传 `loong64` nightly 资产
- 对已有 nightly prerelease 刷新 `hashes.sha256` 与 release notes，不改动 AUR 主链

触发：

- `workflow_run`（来源：`Nightly`）
- `workflow_dispatch`
...
- 2026-08-13 将默认锁定从 `v3.45.0-1.0.pre-198+debian13` 升级到 `v3.46.0-1.0.pre-327`（配合官方 Flutter 3.47.0 依赖整体升级）。**已验证通过**：`nightly-20260813` 的 loong64 构建（QEMU）成功，`flutter pub get` 正常（3.45 的 `native.git` 缺 `pkgs/data_assets` 问题在 3.46 已修复），`riverpod_generator 4.0.8` + freezed 4.0.0-dev.3 + analyzer 13.3 在龙芯上正常生成（未触发 `InvalidTypeException` 容错），bundle + deb 产物正常发布
...
- 正式发版：`amd64 + arm64` 全格式，外加 `loong64` 的 `bundle + deb`
...
linglong-store-<label>-linux-loong64.tar.gz    ← 由 nightly-loong64.yml 异步补传
linglong-store-<label>-linux-loong64.tar.gz.asc
linglong-store-<label>-loong64.deb
...
linglong-store-<version>-linux-loong64.tar.gz + .asc
...
- `render-packaging-templates.sh` 必须显式把 Dart 工具的资源根目录固定到主仓库；AUR 发布会在克隆目录内调用渲染器，禁止依赖调用方当前工作目录定位 `lib/core/i18n/l10n`
...
- 顶层 `run-name`
  ...
  - `nightly-loong64.yml`：仅区分 workflow_run / manual
