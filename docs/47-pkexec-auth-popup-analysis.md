...
>
> **v3 修订（2026-09-10，已被 docs/51 取代）**：所有存在"同 UID 替换 helper"风险的
> 形态（AppImage FUSE/extract-and-run、用户解压 bundle、开发构建）的 install/update
> 改为"普通用户直连 ll-cli"（每任务一次系统授权），不再走 pkexec helper；FUSE
> 暂存机制整体删除。§5.2、§5.2.1、§5.2.2、§10.3、§12、§13、§15、§17.4 中与这些
> 形态/暂存相关的内容仅作历史记录，§5.2.2 记录的"已接受剩余风险"已撤销。
>
...
> **2026-09-10 更新（docs/51）**：上述“接受剩余风险”已撤销——解压 bundle、开发
> 构建与 AppImage 形态不再使用 helper（改为普通用户直连 ll-cli，每任务一次系统
> 授权）；FUSE 暂存机制已删除；系统包形态（DEB/RPM/Copr/AUR）的 helper 行为
> 保持不变。
...
4. §14 的旧 `pkexec kill` 路径仍只服务于未注入 helper 的过渡期直连任务，免密
   路径禁止复用该分支。
5. docs/51 起，非系统包管理器形态（AppImage、解压包、开发构建）的 install/update
   不经 helper、直接以普通用户执行 ll-cli（每任务一次系统授权；取消与该免密路径
   相同，只向本进程启动的 ll-cli 发 SIGTERM）。该回退与免密开关相互独立：
   开启免密时仍优先走免密路径（无弹窗）。
