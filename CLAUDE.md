...
## 变更记录
- 2026-06-14：数据迁移框架 `app_data_migrations` 升级到 0.2.0，**统一接管目录迁移**与业务级数据修补。原 `AppDataDirectoryMigration` 类已删除，目录迁移逻辑封装为 `V001MigrateLegacyDataDirectory` 注册到 `appMigrations`。路径 helper 抽到独立的 `lib/core/storage/app_data_directory_paths.dart`（被 AppLogger、LinuxRendererService 等多处依赖）。Repository 改用框架内置的 `FileMigrationStateRepository`，文件位置 `$XDG_DATA_HOME/<当前AppID>/.migration_state.json`，符合 XDG 标准；不再使用 `SharedPrefsMigrationStateRepository`（已删除）。启动顺序：`AppLogger.init → SingleInstance → WindowService → _runMigrations（FileRepository+FileLock，跑 V001 目录迁移及后续业务 V）→ PreferencesService → ApiClient/CacheService/runApp`。所有迁移逻辑统一在 `_runMigrations` 一次跑完，**必须在任何依赖应用数据目录的服务之前**。
