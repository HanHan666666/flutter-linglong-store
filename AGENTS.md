...
1. **装饰性图标必须排除语义**：纯装饰图标（默认/错误/加载图标等）一律用 `ExcludeSemantics` 包裹，避免屏幕阅读器朗读无意义内容。
2. **骨架屏/加载状态必须标注**：用 `Semantics(label: l10n.loading)` 包裹骨架屏与 shimmer。
3. **列表/网格区域必须标注**：用 `Semantics(label: l10n.a11yAppListArea)` 等语义标签标注容器用途。
4. **错误信息必须支持复制**：`Tooltip` 显示完整内容 + 蓝色「复制」`TextButton`（可 Tab 聚焦、Enter/Space 激活）。
5. **交互控件必须用无障碍封装**：按钮/图标按钮/列表项/Tab/卡片统一使用 `A11yButton`、`A11yIconButton`、`A11yListItem`、`A11yTab`、`A11yCard`，不要手写缺失 `Semantics` 的交互控件。
6. **焦点必须隔离**：页面/弹窗用 `A11yFocusScope` 包裹；KeepAlive 隐藏页用 `ExcludeFocus` 移出焦点树。
...
所有关键业务约定、行为变更与历史决策统一记录在 `CHANGELOG.md`（按时间倒序）；
新增条目一律写入该文件，不在本文件维护第二份日志。
