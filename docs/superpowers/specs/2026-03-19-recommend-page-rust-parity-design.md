...
- 轮播信息锚点与底部指示器必须保持明确安全间距；当前桌面基线约束为 Banner 高度 `236px`，指示器距底 `0px`，禁止出现视觉重叠。
- 背景层必须抽离为独立组件，后续支持替换为图片背景或其他风格实现。
- 背景主色优先从 logo / 图标本身提取，生成同色系调色板；不能继续用标题 hash 或随机色盘决定最终色相，避免出现蓝色 logo 配粉色背景的错配。
...
- 调色板解析逻辑应继续收敛在独立解析器中，便于后续替换为更复杂的图片取色或服务端下发颜色。
