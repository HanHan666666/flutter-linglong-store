...
- 应用名按内容固有宽度完整展示，不设固定像素上限；任何语言（含英语、西语等
  长标题语言）都不出现省略号。
- 标题变宽时由搜索框让位：标题先按固有宽度占位，剩余空间按 1:1 分配给搜索框
  与右侧拖拽区，因此搜索框会右移并收窄（仍满足下节「父容器 50%，最大 534px」）。
- 仅当窗口窄到搜索区连最小可用宽度都保不住时（远低于窗口最小宽度 1280px），
  标题才回退省略号，保证任何窗口宽度下都不会出现 Row 溢出。阈值与推导见
  `CustomTitleBar._titleReservedForSearchWidth`。
