...
- nightly AI changelog 只能输出结构化 JSON 条目，Markdown 标题和 `1、2、3` 编号必须由 `build/scripts/claude-code-release-changelog.sh` 渲染；禁止让 AI 直接输出 release notes 编号列表，避免再次出现 `0、` 起始编号
...
- 正式 release AI changelog 只能输出结构化 JSON 条目，最终 `## Release Notes` 与 `1、2、3` 编号由脚本统一渲染；prompt 中禁止保留一次性“特殊用户要求”、git 提交、删除 prompt 等会污染 CI 的指令
