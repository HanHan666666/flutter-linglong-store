...
## 变更记录
- 2026-06-14：数据迁移框架 `app_data_migrations`（独立仓库 `HanHan666666/app_data_migrations`，开发期 path 依赖，发版后切 pub.dev）已接入。启动流程新增"目录迁移 → 日志 → 单实例 → 窗口 → SharedPreferences → **数据迁移** → ApiClient/CacheService/runApp"步骤，迁移代码位于 `lib/core/migrations/`。注册表 `appMigrations` 手动维护，新增 V 脚本必须实现 `app_data_migrations.Migration` 接口且 `id` 永不复用；跨进程并发由 `FileMigrationLock`（基于 `dart:io` 文件锁，等价 POSIX flock）保证，业务层异常兜底当前策略是"启动失败即 exit(1)"，启动阶段无 UI 上下文，故未实现弹窗"清理所有数据"。**新装场景不专门检测**——新装用户也会执行所有 V 脚本，依靠 V 脚本本身的幂等性兜底（Flyway 默认行为）。**新增 V 脚本的 Code Review 必须按 `app_data_migrations` 仓库的 `doc/03-迁移脚本编写指南.md#v-脚本-review-checklist` 逐项检查**，重点关注幂等性（包括"假设数据本来就干净时 up() 是否安全"）。
