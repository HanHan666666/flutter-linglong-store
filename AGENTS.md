...
- 2026-06-18：OSTree 修复必须兼容不同版本 `ostree fsck` 行为：优先执行 `pkexec ostree fsck --repo=/var/lib/linglong/repo --all --delete`；仅当输出明确表示不支持 `--all` 时，降级重试不带 `--all` 的 `--delete` 命令，并追加写入同一日志。若 `--delete` 不支持，禁止退化成只检查并伪装修复成功，必须提示当前版本无法自动删除损坏对象。新版 OSTree 输出 `partial commits from fsck-detected corruption` 或 `partial commits not verified` 且返回非零码时，应视为自动删除步骤已执行但仍需重新拉取受影响应用/基础环境，UI 要展示具体 partial commit 数量和后续处理建议，不能再显示笼统“修复失败”。
