...
- 2026-06-14：所有用户文件严格遵循 [XDG Base Directory Specification](https://specifications.freedesktop.org/basedir/latest/)。统一入口为 `lib/core/storage/app_xdg_paths.dart` 的 `AppXdgPaths` 类，覆盖 `$XDG_DATA_HOME` / `$XDG_CONFIG_HOME` / `$XDG_CACHE_HOME` / `$XDG_RUNTIME_DIR` 四类目录解析。**禁止再硬编码 `$HOME/.local/share/...` 或 `/tmp/...` 等路径**，新增文件一律走 `AppXdgPaths` 的对应方法。本次整改：
  ...
  - Hive cache 从 `$XDG_DATA_HOME` 迁到 `$XDG_CACHE_HOME`（缓存按规范不属于 data）
  - renderer_preferences.ini 从 `$XDG_DATA_HOME` 迁到 `$XDG_CONFIG_HOME`（V002 自动搬迁老用户）
  - linglong_install_log_service / feedback_dialog / linux_renderer_service 等全部走 `AppXdgPaths`
- 2026-06-14：数据迁移框架 `app_data_migrations` 升级到 0.2.0，**统一接管目录迁移**与业务级数据修补。原 `AppDataDirectoryMigration` 类已删除，目录迁移逻辑封装为 `V001MigrateLegacyDataDirectory` 注册到 `appMigrations`。Repository 改用框架内置的 `FileMigrationStateRepository`，文件位置 `$XDG_DATA_HOME/<当前AppID>/.migration_state.json`，符合 XDG 标准；不再使用 `SharedPrefsMigrationStateRepository`（已删除）。启动顺序：`AppLogger.init → SingleInstance → WindowService → _runMigrations（FileRepository+FileLock，跑 V001 目录迁移及后续业务 V）→ PreferencesService → ApiClient/CacheService/runApp`。所有迁移逻辑统一在 `_runMigrations` 一次跑完，**必须在任何依赖应用数据目录的服务之前**。
- 2026-06-14（早）：数据迁移框架 `app_data_migrations`（独立仓库 `HanHan666666/app_data_migrations`，开发期 path 依赖，发版后切 pub.dev）已接入。**新装场景不专门检测**——新装用户也会执行所有 V 脚本，依靠 V 脚本本身的幂等性兜底（Flyway 默认行为）。**新增 V 脚本的 Code Review 必须按 `app_data_migrations` 仓库的 `doc/03-迁移脚本编写指南.md#v-脚本-review-checklist` 逐项检查**，重点关注幂等性（包括"假设数据本来就干净时 up() 是否安全"）。
