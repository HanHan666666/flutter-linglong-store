...
| 4 | 更 新 | `/update_apps` | 刷新线性图标 + Badge | ✅ |

> "排行"菜单配置了但 `isHidden: true`，不显示。
...
| 图标 | 尺寸 | 路径/操作 |
|------|------|----------|
| 我的应用 (my_apps.svg) | 2rem = 32px | 导航到 `/my_apps` |
| 下载管理 (download.svg) | 2rem = 32px | 弹出下载管理 Modal |
| 设置 (setting.svg) | 2rem = 32px | 导航到 `/setting` |

布局：`flex, justify-content: space-around`

响应式 (≤768px)：改为 `flex-direction: column`
